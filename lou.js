function ltReady() {
    ...
    for (var i = 0; i < events.length; i++)
        for (var j = 0; j < events[i][1].length; j++)
            if ((new Date(events[i][1][j])).toDateString() === today.toDateString())
                current = events[i][0];
    $('#nom').append('<small style="display:block; z-index:10;">« ' + baseline[current][Math.floor(Math.random() * baseline[current].length)] + ' »</small>');
    ...
    $('head').append('<style type="text/css">.mod { position:fixed; background:silver; border-radius:20px; padding:2px; border:1px solid #666; } #chatbox { overflow-x:hidden; display:flex; flex-direction:column-reverse; } @-moz-document url-prefix() { #chattbl { max-height:100%; display:flex; flex-direction:column; } } #chattbl > tr:not(.backlog, .log:first-of-type) { opacity:0; } header { background:' + head[current][Math.floor(Math.random() * head[current].length)] + '; text-align:center; } td:nth-of-type(3) { display:none; } #userlist div, #chat td:nth-of-type(1), #chat td:nth-of-type(1) * { cursor:pointer; -webkit-touch-callout:none; -webkit-user-select:none; -khtml-user-select:none; -moz-user-select:none; -ms-user-select:none; user-select:none; } #userlist, #input, body, tr { background:#f5f5f5 !important; } #chat td:nth-of-type(2) img { max-width:100%; max-height:360px; } #chat td:not(:last-child) { border-right:0; } #input { text-indent:5px; outline:0; border:0; border-top:1px solid #666; } #userlist { margin-bottom:120px; padding-bottom:5px; border-left:0; } #userlist div { border-bottom:0; } #chat { border-right: 1px solid #666; } #nom { margin:0 20%; text-shadow:0 1px 1px #666; padding:5px; font-weight:bold; color:#FFF; position:absolute; left:0; right:185px; } .log td:nth-of-type(2) { text-align:center; } #volrange { position:relative; top:-2px; z-index:5; } #cover { z-index:15; } #window { z-index:20; } #sidebtns { padding-right:22px; margin-right:0; } label:not(.sound) { position:relative !important; top:3px !important; } .sound { font-size:23px; } label + img { position:relative; left:2px; margin-left:0 !important; } img { vertical-align:middle; } #chattbl > tr:nth-of-type(1) { opacity:0.2 !important; } #chattbl > tr:nth-of-type(2) { opacity:0.4 !important; } #chattbl > tr:nth-of-type(3) { opacity:0.6 !important; } #chattbl > tr:nth-of-type(4) { opacity:0.8 !important; } .mod > label { display:none; } .mod:hover > label { display:inline-block; } ' + css[current] + '</style>');
    ...
    $('#prefs').prepend('<div><input type="checkbox" id="preview" checked><label for="preview"> Images et vidéos</label></div><div><input type="checkbox" id="timestamp"><label for="timestamp"> Date et heure</label></div>');
    $('#prefs').append('<div><a href="LaMenuiserie" target="_blank">Salon VIP</a></div>');
    //$('#prefs').append('<div><pre style="background:#FFE; border:2px dashed #666; font-size:12px; padding:1em;">Script VIP</pre></div>');
    $('#volrange').insertAfter('#sidebtns > select');
    $('#prefs div:nth-of-type(4)').remove();

    function improve(i) {
        var p = $('#preview').is(':checked');
        i = i.replace(/[\"&<>]/gi, function(a) {
            return {
                '"': '&quot;',
                '&': '&amp;',
                '<': '&lt;',
        ...
        i = i.replace(/-([^-]*)-/gi, function(b) {
            var i = $.inArray(b.slice(1, -1).toLowerCase(), pokemon);
            return i !== -1 ? ('https://loult.family/pokemon/' + ('000' + (i + 1)).slice(-3) + '.gif') : b;
        });
    ...
    insertionQ('#chattbl > tr').every(function(element) {
        ...
        if (!l) {
            ...
            $(element).children('td:nth-of-type(1)').click(function() {
                $('#input').val($('#input').val() + ' -' + $(element).children('td:nth-of-type(1)').text().trim() + '- ');
            });
        }
        ...
        if ($('#timestamp').is(':checked'))
            $(element).children('td:nth-of-type(3)').show();
        if (!l || (l && !$('#hidelog').is(':checked')))
    ...
    insertionQ('#userlist > div').every(function(element) {
        $(element).children('label:nth-of-type(1)').click(function() {
            $('#input').val($('#input').val() + ' -' + $(this).text() + '- ');
        });
    });
    $(function() {
        $('#userlist > div label:nth-of-type(1)').click(function() {
            $('#input').val($('#input').val() + ' -' + $(this).text() + '- ');
        });
        $('#hidelog').change(function() {
            if ($(this).is(':checked'))
                ...
                $('.log').show();
            setTimeout($('#chatbox').animate({
                scrollTop: $('#chatbox')[0].scrollHeight + 500
            }, 100), 500);
        });
        $('#timestamp').change(function() {
            if ($(this).is(':checked'))
                $('td:nth-of-type(3)').show();
            else
